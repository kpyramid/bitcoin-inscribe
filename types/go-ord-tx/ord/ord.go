...
import (
	...
	extRpcClient "github.com/kpyramid/bitcoin-inscribe/types/go-ord-tx/pkg/rpcclient"
...
type InscriptionTool struct {
	...
	txCtxTransferList         []TransferData
	changePkScript            []byte
...
const (
	...
	DefaultRevealOutValue = defaultRevealOutValue
...
func (tool *InscriptionTool) _initTool(net *chaincfg.Params, request *InscriptionRequest) error {
	...
	// transfer
	tool.txCtxTransferList = request.TransferList
	tool.changePkScript = request.ChangePkScript
	...
	if singleRevealTxOnly {
		...
		for i := 0; i < total; i++ {
			...
			prevOutput := revealOutValue + int64(tx.SerializeSize())*feeRate
			{
				...
				emptyControlBlockWitness := make([]byte, 33)
				fee := (int64(wire.TxWitness{emptySignature, tool.txCtxDataList[i].inscriptionScript, emptyControlBlockWitness}.SerializeSize()+2+3) / 4) * feeRate
				log.Infof("reveal tx fee: %d, witness fee: %d, revealOutValue: %d", prevOutput-revealOutValue, fee, revealOutValue)
...
func (tool *InscriptionTool) getTxOutByOutPoint(outPoint *wire.OutPoint) (*wire.TxOut, error) {
	...
	if tool.svc.Client != nil {
		tx, err := tool.svc.Client.GetRawTransactionVerbose(&outPoint.Hash)
		if err != nil {
			return nil, wrap.WithErrorf(" get raw tx failed by client. error: %s", err)
		}
		...
		log.Infof("GetRawTransaction hash: %s", outPoint.Hash.String())
		tx, err := tool.client.btcApiClient.GetRawTransaction(&outPoint.Hash)
		if err != nil {
			return nil, wrap.WithErrorf(" get raw tx failed by btcApiClient. error: %s", err)
		}
...
func (tool *InscriptionTool) buildCommitTx(commitTxOutPointList []*wire.OutPoint, totalRevealPrevOutput, commitFeeRate int64) error {
	...
	for _, t := range tool.txCtxTransferList {
		tx.AddTxOut(wire.NewTxOut(t.Amount, t.AddressPkScript))
		transferAmount += t.Amount
	}

	// set change pkScript
	if tool.changePkScript == nil {
		// tx.AddTxOut(wire.NewTxOut(0, *changePkScript))
	} else {
		tx.AddTxOut(wire.NewTxOut(0, tool.changePkScript))
	}

	fee := btcutil.Amount(mempool.GetTxVirtualSize(btcutil.NewTx(tx))+17) * btcutil.Amount(commitFeeRate)
	changeAmount := totalSenderAmount - btcutil.Amount(totalRevealPrevOutput) - btcutil.Amount(transferAmount) - fee

	log.Infof("totalSenderAmount: %d, commitFee %d, totalRevealPrevOutput fee: %d, transferAmount: %d, changeAmount: %d", totalSenderAmount, fee, totalRevealPrevOutput, transferAmount, changeAmount)
	log.Infof("min need totalAmount: %d", btcutil.Amount(totalRevealPrevOutput)+btcutil.Amount(transferAmount)+fee)
	if changeAmount > 0 {
		if tool.changePkScript == nil {
			if transferAmount != 0 {
		...
		log.Infof("remove tx out")
		if changeAmount < 0 {
			feeWithoutChange := btcutil.Amount(mempool.GetTxVirtualSize(btcutil.NewTx(tx))) * btcutil.Amount(commitFeeRate)
			needBalance := totalSenderAmount - btcutil.Amount(totalRevealPrevOutput) - feeWithoutChange
			if needBalance < 0 {
...
func (tool *InscriptionTool) GetCommitTxHex() (string, string, error) {
	hexData, err := getTxHex(tool.commitTx)
	if err != nil {
		return "", "", err
	}

	return hexData, tool.commitTx.TxHash().String(), nil
}

func (tool *InscriptionTool) GetRevealTxHexList() ([]string, []string, error) {
	txHexList := make([]string, len(tool.revealTx))
	txHashList := make([]string, len(tool.revealTx))
	for i := range tool.revealTx {
		...
		txHexList[i] = txHex
		txHashList[i] = tool.revealTx[i].TxHash().String()
	}
	return txHexList, txHashList, nil
}
